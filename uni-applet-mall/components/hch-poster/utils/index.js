...
export function drawTextReturnH(
  ...
  for (let n = 0; n < arrText.length; n++) {
    let testLine = line + arrText[n]
		if (bold) {
			ctx.font = 'bold ' + fontSize + 'px sans-serif' //设置字体大小，注意：百度小程序 用ctx.setFontSize设置字体大小后，计算字体宽度会无效
		}else {
  ...
  return y
		 ...
		 * lineNumber: 显示几行
		 */
export function	canvasMultiLineText(ctx, text, contentWidth, lineNumber) {
