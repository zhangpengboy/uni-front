<template>
	<view v-if="hasLogin">
						...
						<view class="goods">
							...
							<view class="item">
								...
								<view class="price-num text-main">
									<view class="left">
										<view class="text-price price-clor">
			...
			<view class="check-total">
				...
				<view v-if="!managementFlag">
					...
					<view class="preferential" @click.stop="showService()">
						...
						<text class="text-main  price-clor">金额明细</text>
		...
		<!-- 优惠-模态层弹窗 -->
		<view class="cu-modal bottom-modal" :class="serviceClass" @tap.stop="hideService">
						...
						<view class="modal-cell">
							...
							<view class="price-num text-r">
								...
								<view class="price-preferential " v-if="vipStatus != 1">
									<text class="price-clor">会员价</text>
									<view class="price-num text-main">
										<view class="text-price price-clor">
	...
	export default {
		...
		methods: {
			...
			async getCartList() {
				...
				this.cartList = this.cartList.concat(goodsCartList);
			...
			handlerJian(goods, index, i) {
				...
				const query = Object.assign({}, goods)
				query.ids = [goods.id]
				query.operNum = -1
				// console.log(query)
			...
			moveFavorites() {
				...
				const that = this;
				let data = this.cartList.map(v => v.list.filter(e => e.active)).filter(v => v.length > 0)
				// console.log(data)
				let cartIds = []
