<template>
	<view class="myWallet">
		<view class="myCd">
			<view class="assets">
				<view class="myAssets">{{$queue.getIntegralName()}}总数 （个）</view>
	...
	export default {
		...
		methods: {
			...
			gotothisdetail(v) {
				let title = '';
				if (v == 1) {
					title = `${this.$queue.getIntegralName()}明细`;
				} else if (v == 2) {
					title = `冻结${this.$queue.getIntegralName()}明细`;
				} else {
					title = `待释放${this.$queue.getIntegralName()}明细`;
				}
				uni.navigateTo({
