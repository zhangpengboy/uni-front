				...
				<view class="main-wrap-box detail">
					<view class="row">
						...
						<view style="color: #333;font-size: 28upx;" class="content text-price">
							<text>{{ buyGoods.sku.salePrice * buyGoods.number }}</text>
						</view>
					</view>
					<view class="row" v-if="buyGoods.sku.integral > 0">
						...
						<view class="content"> <text class="text-xs"
								v-if="!buyGoods.sku.integralType">-</text>{{buyGoods.sku.integral * buyGoods.number }}
						</view>
					</view>
					...
					<view class="row">
						...
						<view class="content">
							...
							<text class="money text-price text-main">
								...
								<text>{{ buyGoods.sku.salePrice * buyGoods.number - preferential  }}</text>
								<template v-if="buyGoods.sku.integral > 0">
									<text class="text-xs">{{ buyGoods.sku.integralType | filterIntegralType }}</text>
									<text>{{buyGoods.sku.integral * buyGoods.number }}</text>
									<text class="text-xs">{{ $queue.getIntegralName() }}</text>
								</template>
							</text>
							<!-- </view> -->
		...
		<template v-else>
					...
					<view class="row">
						<view class="goods-info">
							...
							<view class="info">
								...
								<view class="spec">选择: {{item.skuName }} 数量: x{{ item.num }}</view>
								<view class="price-number">
									<view class="price text-price">
									...
									<view class="number" v-if="item.fail">
										<text class="text-xs text-main">下单失败</text>
									</view>
								</view>
							</view>
						</view>
			...
			<view class="main-wrap">
				<view class="main-wrap-box detail">
					...
					<view class="row" v-if="sumSendIntegral > 0">
						<view class="nominal">赠送{{$queue.getIntegralName()}}</view>
						<view class="content">
							<!-- <text class="text-xs">+</text> -->
							<text>{{ sumSendIntegral }}</text>
						</view>
					</view>
					<view class="row" v-if="sumDikouIntegral > 0">
						<view class="nominal">抵扣{{$queue.getIntegralName()}}</view>
						<view class="content">
							<text class="text-xs">-</text>
							<text>{{ sumDikouIntegral }}</text>
						</view>
					</view>
					<view class="row">
						<view class="nominal">运费</view>
			...
			<view class="footer">
				<view class="settlement">
					<view class="sum">
						...
						<view class="money text-price text-main">
							<text>{{ sumAmount }}</text>
							<template v-if="sumDikouIntegral > 0">
								<text class="text-xs">+</text>
								<text>{{ sumDikouIntegral }}</text>
								<text class="text-xs">{{ $queue.getIntegralName() }}</text>
							</template>
	...
	export default {
		...
		computed: {
			...
			sumSkuAmount: function() {
				if (Object.keys(this.buyGoods).length == 0) return
					return ((this.buyGoods.sku.salePrice * this.buyGoods.number) + +this.buyGoods.base.postage).toFixed(2)
			},
			// 会员优惠
			preferential: function() {
				if (Object.keys(this.buyGoods).length == 0) return
				return ((this.buyGoods.sku.salePrice - this.buyGoods.sku.vipPrice) * this.buyGoods.number).toFixed(2)
			},
			// 总价格
			sumAmount: function() {
				let amount = 0;
				amount = _.sum(this.buylist.filter(v => !v.fail).map(v => (v.salePrice + this.freight) * v.num));
				return amount;
			},
			// 总赠送积分
			sumSendIntegral: function() {
				let amount = 0;
				amount = _.sum(this.buylist.filter(v => !v.fail && v.integralType == 1).map(v => v.integral * v.num));
				return amount;
			},
			// 总抵扣积分
			sumDikouIntegral: function() {
				let amount = 0;
				amount = _.sum(this.buylist.filter(v => !v.fail && v.integralType == 0).map(v => v.integral * v.num));
				return amount;
			},
		},
		filters: {
		...
		methods: {
			...
			async submitOrder() {
				...
				// if (res.code == 200) {
					// uni.hideLoading()
					// if (this.buyGoods.sku) {
					// 	let sumOther = this.buyGoods.sku.integral * this.buyGoods.number
					// 	if (this.buyGoods.sku.integralType == 0) {
					// 		if (!res.success || res.data.freeAmount < sumOther) {
					// 			return that.$queue.showToast(`账号${that.$queue.getIntegralName()}不足!`);
					// 		}
					// 	}
					// }
					// if (!res.success || res.data.freeAmount < that.sumDikouIntegral) {
					// 	return that.$queue.showToast(`账号${that.$queue.getIntegralName()}不足!`);
					// }
					// 提交订单数组
					const orderList = [];
					...
					if (that.isFromCart) {
						...
						_.map(that.buylist, v => {
							...
							if (this.$queue.getJson('giftselectList') != undefined) {
								...
								omsOrderDetail.salePrice = that.buyGoods.sku.salePrice - price < 0 ? 0 : that
									.buyGoods.sku.salePrice - price
								omsOrderDetail.settlePrice = that.buyGoods.sku.settlePrice - price < 0 ? 0 :
									that.buyGoods.sku.settlePrice - price
							}
							if (that.actCode == this.$stringUtils.giftbagCode()) {
								omsOrder.bsType = 'BS_A008'
							}
							if (v.integralType == 0 && omsOrder.bsType == "BS_A001") {
							...
							orderList.push({
								inviteCode: this.$queue.getInvitation(),
						...
						// 判断积分为赠送则不传actCode
						// if (that.buyGoods.sku.integralType == 1) {
						// }
						// 订单信息
						let omsOrder = {
							// actCode: that.actCode,
					...
					let result = await that.$http.createGoodsOrder(orderList)
					uni.hideLoading();
					if (result.code != 200) {
