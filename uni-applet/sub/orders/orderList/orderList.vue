	...
	<view v-if="isloading == 0">
		...
		<view class="order-list">
			<view class="onorder" v-if="list.length == 0">
				<image src="https://lz.static.seprain.com/assets/imgs/no_order.png">
				</image>
					...
					<view class="main-wrap-box">
						...
						<view class="order-list-content">
							...
							<view class="goods-detail">
								<view class="name">
									{{row.goodsName ? row.goodsName : row.omsStreetOrderDetails[0].goodsName}}</view>
								...
								<view class="num num_right">数量： <text class="text-xs">x</text>
									{{row.goodsNum ? row.goodsNum : row.omsStreetOrderDetails[0].goodsNum}}</view>
								<view class="num">单价： <text class="text-xs">¥</text>
									{{row.salePrice ? row.salePrice : row.omsStreetOrderDetails[0].salePrice}}</view>
	...
	export default {
		...
		onLoad(option) {
			...
			if(option.tab == 5) {
				...
				this.listQuery.status = option.tab - 1;
			}
		...
		methods: {
			...
			handleTabNav(tbIndex) {
				...
				if (tbIndex == 0) {
					...
					this.listQuery.status = tbIndex - 1;
				}
				this.getGoodsOrderList();
			},
			/* 提醒商家发货 */
			...
			toOrderDetail(row) {
				if(!this.tabTypeIndex) {
					uni.navigateTo({
						url: '/sub/orders/orderDetail/orderDetail?orderNo=' + row.orderNo
					});
				} else {
					uni.navigateTo({
