<template>
	<view class="apply">
		<view class="top">	
			<view class="store-bg" :style="{background: 'url(' + storeDoc.storeBg + ')  no-repeat;'}"></view>
	...
	export default {
		...
		methods: {
			...
			getStoreInfoAndFavor() {
				this.$http.getStoreInfoAndFavor({
					...
					if (res.code == 200) {
						...
						this.storeDoc.storeBg = res.data.storeDoc.storeBg || 'https://lz.static.seprain.com/assets/imgs/storeBg@2x.png'
						this.storeDoc.storeLogo = res.data.storeDoc.storeLogo || 'https://lz.static.seprain.com/assets/imgs/logo.png'
			...
			async getActGoodsList() {
				...
				const res = await this.$http.getStoreGoodsList(this.queryParams);
				if (res.code == 200) {
