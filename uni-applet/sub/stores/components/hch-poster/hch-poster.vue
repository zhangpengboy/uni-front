...
export default {
	...
	props: {
		...
		posterObj: {
			type: Object,
			default: ()=> ({
				url: `https://cdn.seprain.com/mpapp/share.png?time=${Date.now()}`, //商品主图
				icon: 'https://cdn.seprain.com/mpapp/hch-hyj.png', //醉品价图标
				title: `静默思商城，知名大牌，清库抢购！`, //标题
				discountPrice: `99`, //折后价格
				orignPrice: `299`, //原价
				code: `https://cdn.seprain.com/mpapp/qr.png?time=${Date.now()}` //小程序码
			})
		}
	},
	methods: {
		...
		createCanvasImage() {
			...
			let closeBtn = 'https://cdn.seprain.com/mpapp/close_btn.png';
			console.log(ctx);
			console.log(this);
			...
			wx.getImageInfo({
				...
				fail() {
					...
					uni.showToast({ title: '海报商品图生成失败', duration: 2000, icon: 'none' });
				}
			});
			// 关闭按钮
			wx.getImageInfo({
				...
				fail() {
					_this.canvasFlag = true;
					uni.showToast({ title: '海报关闭按钮生成失败', duration: 2000, icon: 'none' });
				}
			});
			// 关闭按钮 end
			// 海报商品title
			...
			wx.getImageInfo({
				...
				fail() {
					_this.canvasFlag = true;
					uni.showToast({ title: '海报会员价图生成失败', duration: 2000, icon: 'none' });
				}
			});
			// 会员价 图标 end
			//绘制价格
			...
			wx.getImageInfo({
				src: code,
				success(res) {
					ctx.drawImage(res.path, (_this.phoneW - 100) / 2, 430 * scaleH, 100, 100);
					ctx.draw(true);
				},
				fail() {
					_this.canvasFlag = true;
					uni.showToast({ title: '海报小程序码生成失败', duration: 2000, icon: 'none' });
				}
			});
			...
			ctx.font = 'normal bold 14px sans-serif';
			ctx.fillText('静默思商城', (_this.phoneW - 70) / 2, 550 * scaleH);
			// 小程序的名称end
			// 长按/扫描识别查看商品
			ctx.setFontSize(14);
			ctx.setFillStyle('#ff5f33'); //文字颜色：默认黑色
			ctx.font = 'normal 14px sans-serif';
			ctx.fillText('长按/扫描识别查看', (_this.phoneW - 120) / 2, 570 * scaleH);
			// 长按/扫描识别查看商品end
			//绘制保存按钮
			ctx.save();
