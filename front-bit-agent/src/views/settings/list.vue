...
<template>
  <div class="center cards">
    ...
    <div class="main-wrap mt-2 color-bit1" v-for="(v, i) in list" :key="i">
      <div class="main-wrap-cont cards-item">
        ...
        <div class="flex-1 pt-1 ml-2">
          <h2 class="fs-14 mb-2">{{ $lance.filterCardName(v.cardName) }}</h2>
          <p class="fs-12 van-ellipsis">
            {{ v.bankName }}（{{ $lance.filterCardNo(v.cardNo) }}）
          </p>
        </div>
        <van-image
          :src="require('@/assets/images/user/bank-cny@2x.png')"
          height="64"
          width="64"
          fit="contain"
        />
    ...
    >
      <div
        class="main-wrap-cont cards-item"
        @click="toPage('bindWallet', v.bitType)"
      >
        <div class="pt-1 fs-14">{{ v.bitType }}</div>
        <div class="flex-1 pt-1 ml-2">
          <h2 class="fs-14 mb-2">{{ v.bitType }} 地址</h2>
          <p class="fs-12 van-ellipsis">
            {{ v.walletAddress }}
          </p>
        </div>
        <van-image
          :src="require(`@/assets/images/user/bank-bit${v.bitType}@2x.png`)"
          height="64"
          width="64"
          fit="contain"
        />
