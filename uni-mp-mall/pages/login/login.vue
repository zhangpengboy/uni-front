			...
			<view class="cu-dialog">
				...
				<view class="padding-sm text-center">
					...
					<view class="text-gray phone">
						<input type="text" v-model="phone" placeholder="请输入邀请人的手机号" maxlength="11"/>
					</view>
					<button class="confirm-btn-weixin" hover-class="none" @tap="getInviteCode" lang="zh_CN">{{ switchFlag ? '确认': '跳过' }}并授权登录</button>
				</view>
			</view>
		</view>
		<!-- #endif -->
	...
	export default {
		...
		methods: {
			getUserProfileCode() {
				...
				this.$http.getInviteConfig().then(res => {
					if (res.code == 200) {
						if (res.data == 0) {
			...
			getInviteCode() {
				if(this.switchFlag) {
					if(!this.phone) return this.$queue.showToast('手机号不能为空')
					this.$http.getInviteCode({phone:this.phone}).then(res => {
						if (res.code == 200) {
							if (res.data) {
			...
			getUserProfile() {
				...
				uni.getUserProfile({
					...
					success: function(infoRes) {
						...
						console.log('that.inviteCode', that.inviteCode);
						// 保存用户的基础信息
