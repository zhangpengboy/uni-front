...
const goBack = i => {
  if (store.state.isApp) {
    if (router.app._route.path === '/' || router.app._route.path === '/assetsIndex' || router.app._route.path === '/edit') {
      return toAppBaseFun('backapp')
    }
  }
  router.go(i)
}
 ...
 */
const getWeixinCode = () => {
  axios.post(apiUri.APP_WECHAT_AUTH, { url: window.location.href }, res => {
...
// 验证手机号是否为空或是否正确
const isPhone = phone => {
  // const zz = /^(?:(?:\+|00)86)?1(?:(?:3[\d])|(?:4[5-79])|(?:5[0-35-9])|(?:6[5-7])|(?:7[0-8])|(?:8[\d])|(?:9[189]))\d{8}$/
  const zz = /^(?:(?:\+|00)86)?1[1-9]\d{9}$/
  if (!phone) {
    return '手机号不能为空'
  }
  if (!zz.test(phone)) {
