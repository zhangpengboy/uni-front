...
<template>
  <div class='order'>
    ...
    <van-popup v-model="openDelivery" position="bottom" round closeable>
      <p class="fs-16 color-text text-center my-2">商品详情</p>
...
export default {
  ...
  methods: {
    ...
    priceDialog (v) {
      this.open = true
      this.id = v.orderParent
      this.salePrice = v.payAmount
    },
    // 打开确认送达的弹框
    shipmentsDialog (v) {
      let _this = this
      _this.$dialog.confirm({
        ...
        .then(() => {
          _this.$http.post(
            // this.$apiUri.APP_STORE_ORDER_UPDATE_STATUS,
            this.$apiUri.APP_STORE_ORDER_CONFIM,
            {
              parentNo: v.orderParent
              // status: 2
            },
