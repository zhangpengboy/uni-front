<template>
  <div class="orderDetail">
    ...
    <div class="order" v-if="orderData.length > 0">
      ...
      <van-cell title="支付方式" v-if="orderData[0].status !== 0 || orderData[0].status !== 4">
        <template #default>
          <span>在线支付</span>
        </template>
      </van-cell>
    ...
    <div
      ...
      v-if="
        ( orderData.length > 0 ) &&
        (orderData[0].status === 0 || (orderData[0].status === 1 && orderData[0].receivingWay === 0) || orderData[0].status === 2)
      "
    >
       <!-- <van-button
        v-if="applyStatus === 0"
