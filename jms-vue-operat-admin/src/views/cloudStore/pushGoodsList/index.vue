  ...
  <div class="app-container">
    ...
    <el-row :span="24" class="mb8">
      <el-col :span="18" class="d-flex-algin-center">
        ...
        >
          <el-option
            v-for="item in Dict.CLOUD_STORE_GOODS_STATUS"
            :key="item.key"
            :label="item.value"
            :value="item.key"
          >
    ...
    >
      <div class="text" v-if="rowDetail">
        <p>
          ...
          <el-tag
            :type="Dict.CLOUD_STORE_GOODS_STATUS[rowDetail.status || 0].type"
            size="small"
            >{{
              Dict.CLOUD_STORE_GOODS_STATUS[rowDetail.status || 0].value
            }}</el-tag
          >
            ...
            <el-form label-width="100px" label-suffix=":">
              ...
              <el-col :span="12">
                <el-form-item label="店铺名称">
                  <el-input
                    :value="rowDetail.cloudStore.storeName"
                    readonly
                  ></el-input>
                </el-form-item>
              </el-col>
...
import {
  getPushGoodsList,
  delPushGoods,
  exportCloudGoodsExcel,
  downloadGoodsQrcode,
  getIncomeCloudStoreDetail,
  incomeDistribution
} from "@/api/goods";
...
export default {
  ...
  data() {
    return {
      ...
      formIncomeConfig: {
        ...
        storeAmount: null,
        salesmanAmount: null,
        provinceAgentAmount: null,
        cityAgentAmount: null,
        areaAgentAmount: null,
        storeIncome: null,
        salesmanIncome: null,
        provinceAgentIncome: null,
        cityAgentIncome: null,
        areaAgentIncome: null,
        incomeDay: null,
      },
      }, // 分润配置表单数据
  ...
  methods: {
    ...
    handleEarningsDown() {
      const _this =this
      let ids;
      ids = this.ids.map((item) => item.uuid);
      if (ids.length <= 0) {
        return this.msgError("选择的数据没有可删除的商品");
      }
      this.$confirm(
        ...
        .then(function () {
          _this.loadingEarnings = true
          _this.msg = _this.$message({
            message: "收益正在发放,请稍等",
            type: "warning",
            duration: 0,
          });
          return incomeDistribution({ uniqueIds: ids, bsType : "BS_B001" });
        })
        .then(() => {
          _this.getList();
          _this.msgSuccess("已成功发放收益");
          _this.msg.close();
          _this.loadingEarnings = false
        })
        .catch(function () {
          _this.msg.close();
          _this.loadingEarnings = false
    ...
    handleConfig(row) {
      ...
      this.isEdit = false;
      getIncomeCloudStoreDetail({goodsId: row.goodsId}).then(res => {
        this.formIncomeConfig.goodsId = row.goodsId
        if(res.data) {
          this.formIncomeConfig = res.data
          this.formIncomeConfig.config = JSON.parse(this.formIncomeConfig.config)
        }
      })
    },
    ...
    handleExport() {
      ...
      // } // obj.storeId
      exportCloudGoodsExcel(obj, "operator").then((res) => {
