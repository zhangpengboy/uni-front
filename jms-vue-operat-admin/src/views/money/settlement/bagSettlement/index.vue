...
<script>
import { relTreeUserList, settlementDes, userLevelReview } from "@/api/user";
import { displayStoreIncomeDistribution } from "@/api/money";
...
export default {
  ...
  methods: {
    ...
    aKeySettlement(row) {
      ...
      displayStoreIncomeDistribution(obj)
        .then((res) => {
          if (res.code == 500) {
            this.msg.close();
    ...
    getSettlementDes(v) {
      ...
      this.loading = true;
      settlementDes(this.listParams).then((res) => {
        if (res.code == 200) {
    ...
    // 等级评测
    handleReview() {
      userLevelReview({ userLevel: "S" }).then((res) => {
        if (res.code == 200) {
