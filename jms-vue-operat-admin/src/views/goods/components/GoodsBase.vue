      ...
      <el-form class="goods-form"
        ...
        <el-row :gutter="20">
          ...
          <!-- <el-col :span="24" :md="8">
            <el-form-item label="礼券规则" required>
              <el-input
                :placeholder="`请输入商品${form.integralType ? '赠送': '抵扣' }的礼券`"
          ...
          <el-col :span="24"
            ...
            <el-form-item label="商品角标"
                          prop="goodsMark">
              <el-select v-model="form.goodsMark"
                         placeholder="请选择商品角标"
                         class="w-100">
        ...
        <el-form-item>
          <el-button type="primary"
                     ...
                     v-loading.fullscreen.lock="fullscreenLoading">{{ isEdit ? '修改' : '查看' }}，下一步<i class="el-icon-arrow-right el-icon--right"></i></el-button>
        </el-form-item>
      </el-form>
    </el-card>
  </div>
...
export default {
    ...
    'form.secondaryCategoryId': {
      handler: function (nVal, oVal) {
        if (nVal) {
          ...
          this.getGoodsCateList(nVal, 'three')
        }
        if (oVal && nVal != oVal) {
          this.$set(this.form, 'level3CategoryId', '')
          this.$set(this.form, 'level3CategoryName', '')
  ...
  methods: {
    ...
    onBrandChange (v) {
      ...
      this.form.brandName = this.goodsBrandOptions.filter(item => item.brandCode === val)[0].brandName
    },
    /** 图片上传之前判断 */
