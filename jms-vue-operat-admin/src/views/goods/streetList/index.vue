...
export default {
  ...
  methods: {
    ...
    getList () {
      this.loading = true
      getStreetPageList(this.queryParams).then(response => {
    ...
    handleVivew (row) {
      ...
      this.$router.push({ name: 'streetGoodsDetail', query: { goodsId: goodsId } })
    },
    /** 修改按钮操作 */
    handleUpdate (row) {
      const goodsId = row.id || this.ids.map(item => item.id).toString()
    ...
    handleDelete (row) {
      ...
      this.$confirm('是否确认删除该商品?', '警告', {
        ...
        .then(function () {
          return delStreetGoods({ ids: ids, actCode: row.actCode, type: 1 })
        })
        .then(() => {
          this.getList()
    ...
    handleGoodsPass (row) {
      ...
      this.$confirm('是否确认审核该商品?', '警告', {
        ...
        .then(function () {
          return updateStreetGoodsStatus({ ids: ids, status: 2, actCode: row.actCode, type: 1 })
        })
        .then(() => {
          this.getList()
    ...
    handleGoodsReject (row) {
      ...
      this.$confirm('是否确认驳回该商品?', '警告', {
        ...
        .then(function () {
          return updateStreetGoodsStatus({ ids: ids, status: 5, actCode: row.actCode, type: 1 })
        })
        .then(() => {
          this.getList()
    ...
    handleGoodsUp (row) {
      ...
      this.$confirm('是否确认上架该商品?', '警告', {
        ...
        .then(function () {
          return updateStreetGoodsStatus({ ids: ids, status: 3, actCode: row.actCode, type: 1 })
        })
        .then(() => {
          this.getList()
    ...
    handleGoodsDown (row) {
      ...
      this.$confirm('是否确认下架该商品?', '警告', {
        ...
        .then(function () {
          return updateStreetGoodsStatus({ ids: ids, status: 4, actCode: row.actCode, type: 1 })
        })
        .then(() => {
          this.getList()
