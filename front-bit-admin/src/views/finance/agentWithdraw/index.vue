...
<script>
import { getAgentDrawList, auditAgentDraw } from "@/api/finance";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      this.queryParams.drawType = 2
      getAgentDrawList(this.queryParams).then((response) => {
    ...
    handleAgree(row) {
      this.$confirm(
        ...
        .then(function () {
          return auditAgentDraw({ orderNo: row.orderNo, status: 1 });
        })
        .then(() => {
          this.getList();
    ...
    handleReject(row) {
      this.$confirm(
        ...
        .then(function () {
          return auditAgentDraw({ orderNo: row.orderNo, status: 4 });
        })
        .then(() => {
          this.getList();
